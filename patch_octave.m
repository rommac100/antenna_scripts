...
#L = c/(2*f_center*sqrt(er_eff))-(.824*H)*(((er_eff+.3)*(W/H+.264))/((er_eff-.258)*(W/H+.8)))
#L = 5.715*10^-3
...
%% ground plane + substrate calc:
%% using 3 * substrate thickness beyond patch width and length and taking tha max of that for just a closer square.
w_g_w = W*1.9
w_g_l = L*1.9


feed_length = (w_g_l-L)/2
